	...
	public function getProducts() {
		...
		if(isset($products) && count($products)) {			
            foreach ($products as $key => $product) {				
				$search ="https://www.flipkart.com/search?q=".$product['product_name']."&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off";
                $productHTML .= '<article class="col-md-4 col-sm-6">';
                $productHTML .= '<div class="thumbnail product wrapper" style="border-radius:20px">';
                $productHTML .= '<figure>';
                $productHTML .= '<a href="'.$search.'" target="_blank"><img style="margin-top:15px" src="phones/'.$product['image'].'" alt="'.$product['product_name'].'" /></a>';
                $productHTML .= '</figure>';
                $productHTML .= '<div class="caption ">';
                $productHTML .= '<a href="'.$search.'" class="product-name" target="_blank">'.$product['product_name'].'</a>';
                $productHTML .= '<div class="price"><b>₹'.$product['price'].'</b></div><br>';
                $productHTML .= '<h6>Brand : '.$product['brand'].'</h6>';
                $productHTML .= '<h6>Material : '.$product['material'].'</h6>';
                $productHTML .= '<h6>Size : '.$product['size'].' GB</h6>';
				$productHTML .= '<div class="d-flex justify-content-center mt-4"><a href="'.$search.'" target="_blank"><button type="button" class="btn btn-primary" style="padding:5px;font-size:15px"><i class="bi bi-bag-fill"></i> Buy Now</button></a></div>';
